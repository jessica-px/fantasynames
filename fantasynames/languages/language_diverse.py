...
class LanguageDiverse(Language):
    def __init__(self, languages: list[Language] = DEFAULT_RANDOM_LANGUAGES,
                 first_name_prob: list[float] = (),
                 last_name_prob: list[float] = ()):
        super().__init__()
        self.languages = languages
        self.first_name_prob = first_name_prob if first_name_prob else [1 / len(languages) for _ in
                                                                        range(len(languages))]
        self.last_name_prob = last_name_prob if last_name_prob else [1 / len(languages) for _ in
                                                                     range(len(languages))]
